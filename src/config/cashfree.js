...
// Debug logs to ensure env variables are loaded correctly
console.log("Cashfree ENV:", CASHFREE_ENV);
console.log("Cashfree APP ID:", CASHFREE_APP_ID ? CASHFREE_APP_ID.substring(0,8) + "****" : "Missing");
console.log("Cashfree Secret Loaded:", CASHFREE_SECRET_KEY ? "Yes" : "No");
...
const createOrder = async (amount, orderId, customerDetails) => {
  ...
  // Basic validation to prevent invalid payment requests
  if (!amount || amount <= 0) {
  ...
  try {
    ...
    console.log("Creating Cashfree Order:", options.data);
 ...
 * This should be used before crediting wallet
 */
